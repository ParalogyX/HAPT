...
It is clearly seen that some of these features define posture transitions and specifically the minor class SIT_TO_STAND. Despite the fact that the dataset contains 561 features, which is quite a lot, and that they were calculated from 6 raw signals and for sure have some correlation with each other, there will not be feature reduction or feature selection used in the project. For machine learning algorithms even very low variance features can be needed to detect rare classes,
